function x_new=ALM_newiter(A,b,rho,s,c,x0,lambda,eta)
...
for iter=1:100
    ...
    Num = 2^num;
    if Num>32
        fprintf('Num = %d in some iteration of ALM\n', Num);
    end
    ...
    for i=1:Num
        ...
        hxs = sum(-x(gx<0)-s) + sum(x(gx>0)-s) - lambda/rho;
        gf = A'*(A*x-b);
...
%         cvx_end
        ...
        F_rho = norm(A*z-b)^2/2 + rho*max(0,norm(z,1)-hzt-c+lambda/rho)^2;
        if F_rho<funv
            x_new=z;
            funv=F_rho;
        end
