function x_new=EPM_newiter(A,b,rho,s,c,x0)
...
for iter=1:100
    ...
    Num = 2^num;
    if Num>32
        fprintf('Num = %d in some iteration of EPM\n', Num);
    end
    ...
    for i=1:Num
        ...
        cvx_precision low
        variables z(n) t
        minimize((A*z-b)'*(A*z-b)/2 + rho*t - rho*gx'*z)
        subject to
            norm(z,1)-c <= t;
            sum(max(z-s,0))+sum(max(-z-s,0)) <= t;
        cvx_end
...
%     fprintf('EPM: Inner Iter = %d,\t F_rho = %1.3e\n',iter,funv);
